...
function HandCards() {
  ...
  const evaluateHand = () => {
    let cards = [cardClicked6, cardClicked7]; // Add any other cardClicked values if they represent your hand cards.
    let hand = PokerHand.solve(cards);
    let name = hand.name; // E.g., "Flush", "Straight", etc.
    console.log(`Your hand is a ${name}`);

    React.useEffect(() => {
      if (cardClicked6 && cardClicked7) {
        // Add other cardClicked checks if needed.
        evaluateHand();
      }
    }, [cardClicked6, cardClicked7]);
  };
  ...
  return (
    <div className="hand-cards">
      <div className="card" onClick={() => handleMenuClick(6)}>
        {cardClicked6.length === 2 && (
          <img className="card-image" src={cardMapper[cardClicked6]} />
        )}
      </div>
      <div className="card" onClick={() => handleMenuClick(7)}>
        {cardClicked6.length === 2 && (
          <img className="card-image" src={cardMapper[cardClicked7]} />
        )}
      </div>
    </div>
  );
